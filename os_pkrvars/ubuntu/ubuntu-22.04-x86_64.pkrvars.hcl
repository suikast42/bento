...
#boot_command            = ["c<wait>set gfxpayload=keep<enter><wait>linux /casper/vmlinuz quiet autoinstall ds=nocloud-net\\;s=http://{{.HTTPIP}}:{{.HTTPPort}}/ubuntu/ ---<enter><wait30>initrd /casper/initrd<wait><enter><wait>boot<enter><wait>"]
